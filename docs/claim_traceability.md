...
**Claims:** 20 manuscript- and scope-grounded items | **VERIFIED** in the matrix below applies only where this repo session produced direct evidence (see status column); narrative manuscript wording was **not** re-extracted from `inputs/manuscript.pdf` in the QA session.
